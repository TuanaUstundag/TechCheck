...
namespace TechCheck_Admin
{
    public partial class Dashboard : Form
    {
        ...
        private void CihazlariFiltrele(string durum)
        {
            using (SqlConnection baglanti = new SqlConnection(baglantiYolu))
            {
                baglanti.Open();
                ...
                if (durum == "Tümü")
                {
                    sorgu = "SELECT Id, MusteriAd, CihazModel, SeriNo, Ariza, Durum, AtananPersonel, KayitTarihi FROM Cihazlar";
                    da = new SqlDataAdapter(sorgu, baglanti);
                }
                else
                {
                    sorgu = "SELECT Id, MusteriAd, CihazModel, SeriNo, Ariza, Durum, AtananPersonel, KayitTarihi FROM Cihazlar WHERE Durum=@d";
                    da = new SqlDataAdapter(sorgu, baglanti);
                    da.SelectCommand.Parameters.AddWithValue("@d", durum);
                }
        ...
        private void btnMenuPersoneller_Click(object sender, EventArgs e)
        {
        ...
        {
            CihazlariFiltrele("Tümü");
        }

        private void btnBeklemede_Click(object sender, EventArgs e)
        {
            CihazlariFiltrele("Beklemede");
        }

        private void btnTamirde_Click(object sender, EventArgs e)
        {
            CihazlariFiltrele("Tamirde");
        }

        private void btnTeslim_Click(object sender, EventArgs e)
        {
            CihazlariFiltrele("Teslim Edildi");
        }
    }
}