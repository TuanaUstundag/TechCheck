        ...
        private void btnKayitYap_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection baglanti = new SqlConnection(baglantiYolu))
            {
                try
                {
                    ...
                    string sorgu = @"INSERT INTO Kullanicilar (KullaniciAdi, Sifre, KullaniciRolu, Email, Maas, Telefon, ResimYolu) 
                                     VALUES (@user, @pass, @role, @mail, @maas, @telefon, @resimyolu)";

                    SqlCommand komut = new SqlCommand(sorgu, baglanti);

                    // Parametreleri sadece 1 kere ekliyoruz
                    ...
                    komut.Parameters.AddWithValue("@resimyolu", string.IsNullOrEmpty(secilenResimYolu) ? "-" : secilenResimYolu);
